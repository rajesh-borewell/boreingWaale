...
export default function ViewBill() {
    ...
    // 'modern' = colourful A4Preview, 'classic' = black & white PrintBill
    const [billStyle, setBillStyle] = useState('modern');
    ...
    const fetchBill = async () => {
        try {
            ...
            toast.error('Failed to load bill. Please refresh.');
        } finally {
            setLoading(false);
        }
    };
    ...
    const handleUpdate = async () => {
        ...
        try {
            const res = await axios.put(`/bills/${id}`, {
                clientName,
                date,
                items,
                grandTotal
            ...
            setBill(res.data);
            setIsEditing(false);
            toast.success('INVOICE_UPDATED', { id: toastId });
        } catch (err) {
            console.error(err);
    ...
    const handleDownload = async () => {
        const previewEl = document.getElementById('bill-preview-area');
        if (!previewEl) return;
        ...
        try {
            ...
            const canvas = await html2canvas(previewEl, {
                scale: 2,
                useCORS: true,
                backgroundColor: billStyle === 'modern' ? '#f1f5f9' : '#ffffff',
                logging: false,
                width: 850, // Force A4 width for the capture
                onclone: (clonedDoc) => {
                    const clonedEl = clonedDoc.getElementById('bill-preview-area');
                    if (clonedEl) {
            ...
            canvas.toBlob((blob) => {
                ...
                link.download = `RAJESH_BOREWELL_BILL_${bill.billNumber}.jpg`;
                link.href = url;
                        ...
                        {!isEditing ? (
                            <>
                                <button
                                    ...
                                    className="col-span-2 lg:col-span-1 flex items-center justify-center gap-2 bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 px-3 md:px-6 py-2.5 rounded-xl font-mono text-[10px] md:text-[11px] tracking-widest hover:bg-indigo-500 hover:text-white transition-all uppercase"
                                >
                                    <Edit3 className="w-4 h-4" /> EDIT BILL
                                </button>

                                <div className="col-span-2 sm:col-span-1 lg:col-span-1 flex items-center bg-black/60 rounded-xl p-1 border border-white/10">
                                    <button
                                        onClick={() => setBillStyle('modern')}
                                        className={`flex-1 px-4 py-2 rounded-lg text-[10px] font-mono tracking-widest transition-all ${billStyle === 'modern' ? 'bg-indigo-500 text-white glow-violet' : 'text-gray-500 hover:text-gray-300'
                                            }`}
                                    >
                                        MODERN
                                    </button>
                                    <button
                                        onClick={() => setBillStyle('classic')}
                                        className={`flex-1 px-4 py-2 rounded-lg text-[10px] font-mono tracking-widest transition-all ${billStyle === 'classic' ? 'bg-gray-800 text-white' : 'text-gray-500 hover:text-gray-300'
                                            }`}
                                    >
                                        CLASSIC
                                    </button>
                                ...
                                <button
                                    ...
                                    className="col-span-2 sm:col-span-1 lg:col-span-1 flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-500 to-teal-600 text-white px-3 md:px-6 py-2.5 rounded-xl font-mono text-[10px] md:text-[11px] tracking-widest shadow-lg hover:shadow-emerald-500/20 transition-all uppercase"
                                >
                            ...
                            <>
                                <button
                                    ...
                                    className="col-span-1 flex items-center justify-center gap-2 bg-red-500/10 text-red-500 border border-red-500/20 px-4 py-2.5 rounded-xl font-mono text-[10px] tracking-widest hover:bg-red-500 hover:text-white transition-all uppercase text-center"
                                >
                                ...
                                <button
                                    ...
                                    disabled={saving}
                                    className="col-span-1 flex items-center justify-center gap-2 bg-green-600 text-white px-4 py-2.5 rounded-xl font-mono text-[10px] tracking-widest shadow-lg transition-all disabled:opacity-50 uppercase text-center"
                                >
                ...
                {isEditing && (
                    ...
                    >
                        ...
                        <div className="flex items-center gap-3 border-b border-white/5 pb-6">
                            ...
                            <div>
                                <h2 className="text-2xl font-black text-white italic tracking-tighter uppercase">CHANGING_BILLNO: #{String(bill.billNumber).padStart(4, '0')}</h2>
                        ...
                        <div className="hud-card hologram-border rounded-3xl overflow-hidden bg-black/60 shadow-2xl relative">
                            ...
                            <div className="p-4 md:p-20 flex justify-center w-full">
                                <div className="w-full overflow-x-auto pb-10 custom-scrollbar flex justify-start md:justify-center">
                                    <div id="bill-preview-area" 
                                        style={{ minWidth: '850px' }}
                                        className={`shadow-[0_0_50px_rgba(0,0,0,0.5)] transform-gpu md:hover:scale-[1.005] transition-transform duration-500 ${billStyle === 'modern' ? 'bg-slate-100 p-0' : 'bg-white p-12 border border-black/5'
                                        }`}>
                                        {billStyle === 'modern' ? (
                                            <A4Preview bill={bill} />
                                        ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    <div className="hud-card p-6 rounded-2xl border-white/5 bg-white/5 backdrop-blur-md">
                        <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className="text-[12px] font-mono text-gray-400 uppercase tracking-widest">BILL CREATED ON</span>
                        </div>
                        <p className="text-white font-mono text-xs">{new Date(bill.createdAt).toLocaleString('en-GB').replace(/\//g, '-')}</p>
                    </div>
                    <div className="hud-card p-6 rounded-2xl border-white/5 bg-white/5 backdrop-blur-md">
                        <div className="flex items-center gap-3 mb-2">
                            <RotateCcw className="w-4 h-4 text-cyan-400" />
                            <span className="text-[12px] font-mono text-gray-400 uppercase tracking-widest">BILL UPDATED ON</span>
                        </div>
                        <p className="text-white font-mono text-xs">{new Date(bill.updatedAt).toLocaleString('en-GB').replace(/\//g, '-')}</p>
                    </div>
                    <div className="hud-card p-6 rounded-2xl border-white/5 bg-white/5 backdrop-blur-md flex flex-col justify-center">
                        ...
                        <p className="text-[12px] text-indigo-300 font-mono italic font-bold uppercase">{bill.createdBy?.username || 'ANONYMOUS_VOID'}</p>
                    </div>
                </div>
            </div>
        </div>
