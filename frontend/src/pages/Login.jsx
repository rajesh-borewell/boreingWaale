...
import { Lock, User, ChevronRight, Binary, ShieldCheck, Sparkles } from 'lucide-react';
import toast from 'react-hot-toast';
import Background3D from '../components/Background3D';

export default function Login({ setAuthInfo }) {
    ...
    const handleLogin = async (e) => {
        ...
        try {
            ...
            toast.success(`Access Granted: Welcome, ${user}`);
            navigate('/');
        } catch (err) {
    ...
    return (
        <div className="relative min-h-screen flex items-center justify-center p-6 overflow-hidden bg-black">
                    ...
                    >
                        <div className="glassmorphism void-distortion rounded-[2.5rem] p-8 md:p-12 border-white/5 singularity-glow backdrop-blur-3xl shadow-[0_0_80px_rgba(0,0,0,0.8)]">
                            <div className="text-center mb-10">
                                ...
                                >
                                    <Lock className="text-white w-10 h-10 text-glow animate-pulse" />
                            ...
                            <form onSubmit={handleLogin} className="space-y-8">
                                <div className="space-y-3">
                                    ...
                                    <div className="relative group/input">
                                        <div className="absolute inset-y-0 left-0 pl-5 flex items-center pointer-events-none">
                                            <User className="w-5 h-5 text-indigo-500/50 group-focus-within/input:text-indigo-400 transition-colors" />
                                        </div>
                                        <input
                                            type="text"
                                            required
                                ...
                                >
                                    <span>{loading ? 'AUTHORIZING...' : 'ENTER'}</span>
                            ...
                            <div className="mt-14 pt-12 border-t border-white/5 text-center">
                                ...
                                >
                                    New User? <span className="text-white font-black ml-2 underline underline-offset-8 decoration-indigo-500/50 group-hover:decoration-indigo-500 group-hover:text-indigo-300 transition-all">Sign Up</span>
                                </button>
                            </div>
                        </div>
                    </motion.div>
            ...
            {/* Static HUD */}
            <div className="absolute top-10 right-10 flex flex-col items-end gap-2 opacity-20 select-none">
                <Binary className="w-6 h-6 text-violet-500" />
                <div className="h-[2px] w-32 bg-gradient-to-l from-violet-500 to-transparent" />
